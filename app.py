...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Filters */
  .filters { padding: 12px 24px; background: #0f0f0f; border-bottom: 1px solid #1a1a1a; display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
  .filter-label { font-size: 11px; color: #555; text-transform: uppercase; letter-spacing: 0.06em; margin-right: 4px; }
  ...
  .col-body.drag-over { background: #141414; border-radius: 8px; }

  /* Cards */
  .card { background: #141414; border: 1px solid #1f1f1f; border-radius: 8px; padding: 12px; margin-bottom: 8px; cursor: grab; transition: border-color 0.15s, transform 0.1s; position: relative; }
  .card:hover { border-color: #2f2f2f; }
  .card.dragging { opacity: 0.4; transform: scale(0.97); }
  .card-name { font-size: 13px; font-weight: 500; color: #e8e8e8; margin-bottom: 8px; line-height: 1.3; }
  ...
  .btn-ghost { background: transparent; color: #888; border: 1px solid #2a2a2a; padding: 8px 14px; border-radius: 6px; font-size: 13px; cursor: pointer; }
  .btn-ghost:hover { border-color: #444; color: #ccc; }

  /* Stats bar */
  .stats { padding: 10px 24px; background: #0a0a0a; border-bottom: 1px solid #141414; display: flex; gap: 20px; flex-wrap: wrap; }
  ...
  .modal { background: #141414; border: 1px solid #2a2a2a; border-radius: 12px; padding: 24px; width: 420px; max-width: 95vw; }
  ...
  .form-group input, .form-group select, .form-group textarea {
    ...
    color: #e8e8e8; font-size: 13px; padding: 8px 10px; outline: none;
  }
  ...
  /* Toast */
  .toast { position: fixed; bottom: 24px; left: 50%; transform: translateX(-50%); background: #1f1f1f; border: 1px solid #2a2a2a; color: #ccc; font-size: 13px; padding: 10px 18px; border-radius: 8px; z-index: 300; opacity: 0; transition: opacity 0.2s; pointer-events: none; }
...
<div class="filters">
  ...
  <button class="filter-btn" onclick="setFilter('category','Product',this)">Product</button>
  <button class="filter-btn" onclick="setFilter('category','Customer',this)">Customer</button>
  ...
  <button class="filter-btn" onclick="setFilter('type','Video',this)">Video</button>
  <button class="filter-btn" onclick="setFilter('type','Pitch Deck',this)">Pitch Deck</button>
  <button class="filter-btn" onclick="setFilter('type','Brochure',this)">Brochure</button>
  <button class="filter-btn" onclick="setFilter('type','Thought Leadership WP',this)">WP</button>
</div>
...
<div class="modal-overlay" id="modalOverlay">
  <div class="modal">
    ...
    <div class="form-group">
      ...
      <input type="text" id="fQuarter" placeholder="e.g. AMJ, JAS">
    </div>
    <div class="form-group">
...
let cards = [];
let filters = { category: "All", type: "All" };
let editingId = null;
let dragId = null;
...
function render() {
  ...
  const closed = cards.filter(c => c.stage === "Closed").length;
  const inProgress = cards.filter(c => !["Not Started","Closed"].includes(c.stage)).length;
  ...
  STAGES.forEach(stage => {
    ...
    col.innerHTML = `
      ...
      <div class="col-body" id="col-${stage.replace(/\s+/g,'-')}"
        ondragover="onDragOver(event)" ondrop="onDrop(event,'${stage}')" ondragleave="onDragLeave(event)">
        ${stageCards.map(c => cardHTML(c)).join("")}
...
function cardHTML(c) {
  const catClass = `tag-cat-${c.category.toLowerCase()}`;
  return `
    <div class="card" draggable="true" id="card-${c.id}"
      ...
      ${c.quarter ? `<div class="card-meta"><span class="quarter-badge">${c.quarter}</span></div>` : ""}
...
async function onDrop(e, stage) {
  ...
  await fetch(`/api/cards/${dragId}`, {
    ...
    body: JSON.stringify({...card, stage})
  });
  await fetchCards();
...
async function saveCard() {
  const payload = {
    ...
    quarter: document.getElementById("fQuarter").value.trim(),
